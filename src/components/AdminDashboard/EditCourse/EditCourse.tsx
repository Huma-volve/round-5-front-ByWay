  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get(
        `https://round5-byway.huma-volve.com/api/courses/${id}`,
        {
          headers: {
    ...
    <div className="container space-y-4 min-h-screen">
      ...
      <Formik
        ...
        onSubmit={async (values, { setSubmitting }) => {
          try {
            await axios.put(
              `http://round5-byway.huma-volve.com/api/courses/${id}`,
              values,
              {
                headers: {
                  Accept: "application/json",
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Field
                    ...
                    name="title"
                    placeholder={t(
                      "instructor.courseManagement.placeholders.courseTittle"
                    )}
                  />
                  <ErrorMessage
                    name="title"
