...
export default function Favourites() {
  ...
  const breadcrumbItems = useMemo(
    () => [{ label: "common.home", link: "/" }, { label: "common.favourites" }],
    []
  );
  const [activeButton, setActiveButton] = useState<"btn1" | "btn2">("btn1");
  const { mutate: removeFavorite, error: removeError } = useRemoveFavorites();
    ...
    <div className="bg-background container">
      <div className="ml-4">
        ...
        {isLoading ? (
          ...
          <div className="mt-5 bg-border rounded-xl w-full md:w-[60%] lg:w-[50%] flex flex-col gap-1 mr-4">
            {favourites.map((fav, index) => (
              <div key={fav.id}>
                {index > 0 && (
                  <hr className="w-[95%] mx-auto h-[3px] bg-secondary opacity-30" />
                )}
                <div className="w-full flex items-center justify-around gap-4 px-4 py-2">
                  <img
                  ...
                  <Heart
                    ...
                    className="w-5 h-5 md:w-6 md:h-6 cursor-pointer flex-shrink-0 ml-auto rtl:mr-auto"
                  />
                </div>
              </div>
            ))}
